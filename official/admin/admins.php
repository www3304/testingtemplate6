...
$admin_query = "SELECT id, username, created_at, updated_at FROM admins ORDER BY id ASC";
$admin_result = $official_admin_connection->query($admin_query);
            ...
            <main class="flex-1 p-6">
                ...
                <div class="mb-6 flex flex-col md:flex-row md:items-center md:justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-500">Manage all registered administrators</p>
                    </div>
                    <div class="mt-4 md:mt-0 flex space-x-3">
                        <a href="add_admin.php" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500">
                ...
                <div class="mb-6">
                    <div class="relative">
                        ...
                        <input type="text" id="adminSearch" class="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md leading-5 bg-white placeholder-gray-500 focus:outline-none focus:placeholder-gray-400 focus:ring-1 focus:ring-primary-500 focus:border-primary-500 sm:text-sm" placeholder="Search admins by username...">
                    </div>
                </div>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                if ($admin_result && $admin_result->num_rows > 0) {
                                    while ($admin = $admin_result->fetch_assoc()) {
                                        if (strtolower($admin['username']) === 'superadmin') continue; // Hide superadmin
                                ?>
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        <?php echo formatDateTime($admin['updated_at']); ?>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex justify-end items-center h-full space-x-2">
                                            <a href="edit_admin.php?id=<?php echo $admin['id']; ?>" class="text-primary-600 hover:text-primary-900" title="Edit Admin">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                                                </svg>
                                            </a>
                                            <!-- Delete Button -->
                                            <button onclick="deleteAdmin(<?php echo $admin['id']; ?>, '<?php echo htmlspecialchars(addslashes($admin['username'])); ?>')" class="text-red-600 hover:text-red-900" title="Delete Admin">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                <tr>
                                    <td colspan="5" class="px-6 py-4 text-center text-sm text-gray-500">
                                        No administrators found. <a href="add_admin.php" class="text-primary-600 hover:text-primary-900">Add the first one</a>.
                                    </td>
                                </tr>
                                <?php    
                                }
    ...
    <script>
        $(document).ready(function() {
            ...
            var table = $('#adminTable').DataTable({
                ...
                order: [[ 0, 'asc' ]], // Default sort by Admin ID 
                ...
                language: {
                    ...
                    emptyTable: 'No admins found. <a href="add_admin.php" class="text-primary-600 hover:text-primary-900">Add the first admin</a>.',
                    paginate: {
                        first: "First",
                        last: "Last",
                        next: "Next",
                 ...
                 columnDefs: [
                     // Disable sorting for Username, Action (indices 1, 4)
                    { targets: [1, 4], orderable: false }, 
                     // Define columns that are searchable (ID and Username)
        ...
        // Function to handle admin deletion confirmation
        function deleteAdmin(adminId, username) {
            if (confirm(`Are you sure you want to delete the admin "${username}" (ID: ${adminId})? This action cannot be undone.`)) {
